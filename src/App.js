    ...
    return (
      <div className="container">
        ...
        <form>
          ...
          <input name="password" value={this.state.password} onChange={this.updatePassword}></input>
        </form>
        <div className="keypad">
          {Object.keys(keypad).map((key, i) => (
            <button key={i} name={keypad[key].num} value={keypad[key].keys} onClick={this.updatePassword}>{keypad[key].num}<br />{keypad[key].keys}</button>
          ))}
        </div>
      </div>
    );
